...
function session (fastify, options, next) {
  ...
  fastify.decorateRequest('originalSession', {})
      ...
      if (decryptedSessionId === false) {
        ...
        options.store.get(decryptedSessionId, (err, session) => {
          ...
          if (session && session.expires && session.expires <= Date.now()) {
            options.store.destroy(sessionId, getDestroyCallback(secret, request, reply, done, cookieOpts), {request: request, reply: reply})
            return
          }
          ...
          request.originalSession = Object.assign({}, session);
          done()
...
function shouldSaveSession (request, cookieOpts, saveUninitialized) {
  if (!saveUninitialized && !isSessionModified(request.session, request.originalSession)) {
    return false
  }
...
function isSessionModified (session, originalSession) {
  for(let key in session){
    if(key === 'expires')continue;
    if(key === 'cookie')continue;
    if(key === 'sessionId')continue;
    if(key === 'encryptedSessionId')continue;

    if(!equal(session[key], originalSession[key]))return true;
  }
